...
- 초안 대시보드 페이지 UI (`src/app/(admin)/admin/drafts/page.tsx`)
  ...
  - 상태별 필터링 탭 (미생성, 생성됨, 게시완료)
  - "초안 생성" 버튼 (개별/일괄)
  - 초안 미리보기 모달/사이드 패널
  - 초안 상태 변경 드롭다운
  - 클립보드 복사 버튼
- DraftPreview 컴포넌트 (`src/components/DraftPreview.tsx`)
  - 생성된 초안 내용 표시
  - 생성 타임스탬프 표시
  - 클립보드 복사 버튼 (성공/실패 토스트)
  - "전체 복사" 및 "문단별 복사" 옵션
  - 초안 편집 가능 영역 (모달 내)
- 관리자 네비게이션 및 레이아웃 (`src/app/(admin)/layout.tsx`)
  - 관리자 헤더 (로고, 사용자명, 로그아웃 버튼)
  - 사이드바 (초안 대시보드, 설정, 로그아웃 링크)
  - 현재 페이지 표시 (breadcrumb)
- 더미 데이터 활용
  - 다양한 상태의 포스트 10-15개 표시
  - 초안 생성 완료/대기 상태 혼합
  - 긴 콘텐츠 및 짧은 콘텐츠 샘플 포함
