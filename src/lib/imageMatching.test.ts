...
describe("imageMatching", () => {
  ...
  describe("matchImagesToParagraphs", () => {
    ...
    it("캡션이 '회'여도 본문이 '사시미'라고 쓰면 동의어로 인정해 매칭된다 (회귀 테스트)", () => {
      // 실측 확인된 사고: 사시미 5점이 정확히 찍힌 사진이 있는데도(비전 캡션이 "회"로
      // 생성됨), 본문은 "사시미"라고 써서 순수 부분문자열 매칭으로는 전혀 겹치지 않아
      // 세 번의 재생성 모두 이 사진이 매칭되지 않고 엉뚱한 사진이 대신 붙었다. 두
      // 표현 다 자연스러운 한국어라 어느 쪽이 나올지 예측할 수 없으므로 동의어로
      // 취급해야 한다.
      const result = matchImagesToParagraphs(
        [{ caption: "회, 모둠" }],
        [{ index: 1, text: "사시미 5점이 먼저 나오는데 퀄리티가 꽤 좋다" }]
      )

      expect(result).toEqual([1])
    })

    it("반대로 캡션이 '사시미'이고 본문이 '회'라고 써도 동의어로 매칭된다 (회귀 테스트)", () => {
      const result = matchImagesToParagraphs(
        [{ caption: "사시미, 광어" }],
        [{ index: 1, text: "신선한 회가 먼저 나왔다" }]
      )
