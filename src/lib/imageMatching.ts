...
- 키워드: 그 사진의 핵심 피사체를 2~4개의 짧은 한국어 단어로 쉼표 구분해서 나열 (예: "우니, 초밥, 클로즈업"). 반드시 한국 블로거가 실제로 쓰는 일상적인 한국어 음식 이름을 쓰고, "자완무시"(→계란찜), "타다키"(→겉불에 살짝 구운 회), "오토로"(→참치 뱃살) 같은 격식체/일본어 원어 표기는 쓰지 마세요 — 본문 캡션 매칭이 문단 텍스트의 실제 단어와 겹치는지로 이뤄지는데, 블로그 본문은 항상 일상 한국어 명칭을 쓰기 때문에 격식체 키워드는 매칭에 실패해 사진이 엉뚱한 문단에 배치되는 원인이 됩니다. 또한 "새우", "회", "참치"처럼 조리법/부위 구분 없이 재료명만 단독으로 쓰지 마세요 — 같은 재료라도 조리법·부위가 다르면(예: 볶은 새우 요리 vs 회로 먹는 단새우, 참치 뱃살 vs 참치 살코기) 전혀 다른 사진인데, 재료명만 쓰면 그 재료가 언급된 다른 문단과 잘못 매칭됩니다. 조리법/형태/부위를 붙인 구체적인 이름을 쓰세요(예: 볶은 새우 요리라면 "새우볶음"처럼 붙여쓰기, 날것 그대로의 단새우라면 "단새우", 지방이 많은 붉은 참치 살이라면 "참치 뱃살", 담백한 참치 살코기라면 "참치 살코기").
